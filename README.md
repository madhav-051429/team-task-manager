...
The `DATABASE_URL` is automatically set by the PostgreSQL plugin.
...
> **Railway URL**: _[Add your deployed URL here after deployment]_
...
1. Open the app → Login as `admin@team.com`
2. **Dashboard**: Show metric cards (7 tasks, 1 overdue), upcoming deadlines, project overview
3. **Projects**: Show project list with progress bars
4. Click into a project → Show Kanban board with task cards
5. Click "+ New Task" → Create a new task, assign to Alice
6. Edit an existing task → Change assignee
7. Delete a task

### Scenario 2: Member Flow
1. Logout → Login as `alice@team.com`
2. **Dashboard**: Show personal metrics (only assigned tasks)
3. **Projects**: Show only projects with assigned tasks
4. Click into a project → Kanban board (no admin controls visible)
5. Click a task → Can only change status (no edit/delete)
6. Move task from "To Do" → "In Progress" → "Done"
7. Try accessing admin features via API → Show 403 response
